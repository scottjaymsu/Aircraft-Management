...
exports.getAirportMarkers = async (req, res) => {
    ...
    */
    ...
    const query = `SELECT ap.Airport_Code AS ident,a.latitude_deg, a.longitude_deg,
        ...
        (SELECT IFNULL(SUM(Area_ft2), 0) / 7.5 FROM airport_parking WHERE Airport_Code = ap.Airport_Code) AS capacity,
        CASE
            WHEN (SELECT IFNULL(SUM(Area_ft2), 0) / 7.5 FROM airport_parking WHERE Airport_Code = ap.Airport_Code) = 0 THEN 'Undercapacity'
            WHEN IFNULL(SUM(at.parkingArea * 1.1), 0) / (SELECT IFNULL(SUM(Area_ft2), 0) / 7.5 FROM airport_parking
        ...
        END AS capacity_status,
        CASE
            WHEN (SELECT IFNULL(SUM(Area_ft2), 0) / 7.5 FROM airport_parking
                WHERE Airport_Code = ap.Airport_Code) = 0 THEN 0
            ELSE ROUND(IFNULL(SUM(at.parkingArea * 1.1), 0) / (SELECT IFNULL(SUM(Area_ft2), 0) / 7.5 FROM airport_parking
